...
def main():
    ...
    rs_topic_depth_img = settings.topic_depth_img
    rs_topic_intrinsics = settings.topic_camera_intrinsics
    ...
    try:
        ...
        if len(ctx.devices) == 0:
            logger.error("No RealSense device connected. Exiting.")
            return

        device = ctx.devices[0]
        logger.info(f"Found RealSense device: {device.get_info(rs.camera_info.name)} (S/N: {device.get_info(rs.camera_info.serial_number)})")
