# Prompt erstellt von
**Person die den Prompt erstellt hat:* Felix Deigner* 
...
# Nutzungsprotokoll
## Verwendete Prompt:
```Hilf mir bitte, mein Projekt so fertig zu machen, dass die Inference mit einer Webcam über Eclipse Zenoh läuft, aber alles nur im src-Ordner. Nutze src/config.py für einfache Settings (Zenoh-Endpoints, Kamera-Index, FPS, Modellname,...) und nutze src/camera/camera_main.py für die Webcam. camera_main.py soll die Webcam mit cv2.VideoCapture(0) lesen, auf 640x480 setzen, als JPEG encodieren und ungefähr 10 FPS auf litter/frame publishen. Die Inference soll die Bilder von litter/frame über Zenoh abonnieren, decodieren```

## Antwort der KI
Alle grundlegende funktionen wurden aus der Vorlesungg übernommen 
KI hat dann aufgeräumt
## Wofür wurde die Antwort genutzt und inwiefern wurde sie überarbeitet ?
Es muste kaum noch etwas gemacht werden:
ein import hat noch gefehlt
und kleine erklärungen waren noch nötig
